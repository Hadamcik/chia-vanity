...
export default function App() {
    ...
    useEffect(() => {
        ...
        const setup = async () => {
            unlistenProgress = await listen<SearchProgressPayload>(
                'search-progress',
                (event) => {
                    setChecked(event.payload.checked);
                    setRatePerSec(event.payload.ratePerSec);
                    setElapsedSecs(event.payload.elapsedSecs);
                    setStatus('Searching...');
                    setUiState((prev) => (prev === 'stopping' ? prev : 'running'));
                },
            );

            unlistenCompleted = await listen<SearchCompletedPayload>(
                'search-completed',
                (event) => {
                    setHit(event.payload.hit);
                    setStatus(event.payload.hit ? 'Match found' : 'No match found');
                    setUiState('idle');
                },
            );

            unlistenFailed = await listen<SearchFailedPayload>(
                'search-failed',
                (event) => {
            ...
            unlistenState = await listen<SearchStatePayload>('search-state', (event) => {
            ...
            const state = await invoke<SearchStatePayload>('get_search_state');
